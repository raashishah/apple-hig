...
`;
    ...
    const checks = {
      ...
      stripWallet: stripStatus.topics.wallet?.state === "pending",
...

{
  let ok = false;
  let detail = {};
  const passDir = fs.mkdtempSync(path.join(os.tmpdir(), "hig-apply-wlt-pass-"));
  const cleanDir = fs.mkdtempSync(path.join(os.tmpdir(), "hig-apply-wlt-clean-"));
  const fixDir = fs.mkdtempSync(path.join(os.tmpdir(), "hig-apply-wlt-fix-"));
  const holdDir = fs.mkdtempSync(path.join(os.tmpdir(), "hig-apply-wlt-hold-"));
  const svgDir = fs.mkdtempSync(path.join(os.tmpdir(), "hig-apply-wlt-svg-"));
  const plainDir = fs.mkdtempSync(path.join(os.tmpdir(), "hig-apply-wlt-plain-"));
  const overDir = fs.mkdtempSync(path.join(os.tmpdir(), "hig-apply-wlt-over-"));
  const padDir = fs.mkdtempSync(path.join(os.tmpdir(), "hig-apply-wlt-pad-"));
  const sentenceDir = fs.mkdtempSync(path.join(os.tmpdir(), "hig-apply-wlt-sentence-"));
  const bareDir = fs.mkdtempSync(path.join(os.tmpdir(), "hig-apply-wlt-bare-"));
  const payDir = fs.mkdtempSync(path.join(os.tmpdir(), "hig-apply-wlt-pay-"));
  const dirs = [passDir, cleanDir, fixDir, holdDir, svgDir, plainDir, overDir, padDir, sentenceDir, bareDir, payDir];
  try {
    const src = path.join(pluginRoot, "eval", "fixtures", "chrome-pass");
    for (const dir of dirs) fs.cpSync(src, dir, { recursive: true });
    const writePassKit = (dir) => {
    ...
    for (const dir of [cleanDir, fixDir, holdDir, svgDir, plainDir, overDir, padDir, sentenceDir]) {
      writePassKit(dir);
    }
    fs.writeFileSync(path.join(payDir, "Pay.swift"), "let button = PKPaymentButton()\n");
    const marked = `export function HostWidgets() {
  return (
    <div data-wallet data-wl-strip>
      <button type="button">Pass</button>
    </div>
  );
}
...
`;
    fs.writeFileSync(path.join(holdDir, "HostWidgets.tsx"), origHold);
    const origSvg = `export function HostWidgets() {
  return (
    <div data-wallet>
      <svg className="strip" aria-hidden="true">
        <text>20% OFF</text>
      </svg>
    </div>
  );
...
`;
    fs.writeFileSync(path.join(svgDir, "HostWidgets.tsx"), origSvg);
    const origPlain = `export function HostWidgets() {
  return (
    <div data-wallet>
      <svg className="strip" aria-hidden="true">
...
`;
    fs.writeFileSync(path.join(plainDir, "HostWidgets.tsx"), origPlain);
    const origOver = `export function HostWidgets() {
  return (
    <div data-wallet>
      <img className="strip" alt="storefront" src="strip.png" />
      <p>20% off</p>
    </div>
  );
}
`;
    fs.writeFileSync(path.join(overDir, "HostWidgets.tsx"), origOver);
    const origPad = `export function HostWidgets() {
  return (
    <div data-wallet>
      <img alt="logo" style="padding: 8px" src="logo.png" />
...
`;
    fs.writeFileSync(path.join(padDir, "HostWidgets.tsx"), origPad);
    const origSentence = `export function HostWidgets() {
  return <p>Avoid embedding text in the strip image.</p>;
}
`;
    fs.writeFileSync(path.join(sentenceDir, "HostWidgets.tsx"), origSentence);
    const origBare = `export function HostWidgets() {
  return <span data-wl-strip>Pass</span>;
}
`;
    fs.writeFileSync(path.join(bareDir, "HostWidgets.tsx"), origBare);
    const names = ["pass", "clean", "fix", "hold", "svg", "plain", "over", "pad", "sentence", "bare", "pay"];
    const dirBy = {
      pass: passDir,
      clean: cleanDir,
      fix: fixDir,
    ...
    const fixed = fs.readFileSync(path.join(fixDir, "HostWidgets.tsx"), "utf8");
    const held = fs.readFileSync(path.join(holdDir, "HostWidgets.tsx"), "utf8");
    const svg = fs.readFileSync(path.join(svgDir, "HostWidgets.tsx"), "utf8");
    const paid = fs.readFileSync(path.join(payDir, "HostWidgets.tsx"), "utf8");
    const bared = fs.readFileSync(path.join(bareDir, "HostWidgets.tsx"), "utf8");
    const hostText = dirs.flatMap((dir) => walkSource(dir)).map((f) => f.text).join("\n");
    const catalog = loadCatalog(skillRoot);
    ...
    const checks = {
      requiredIds: loadSurfaces(skillRoot).requiredIds.length === 12,
      heuristic: (catalog.byId.wallet?.dontHeuristicIds || []).includes("wl-strip"),
      passChrome: reports.pass.chrome.pass === true,
      cleanChrome: reports.clean.chrome.pass === true,
      fixChrome: reports.fix.chrome.pass === true,
      holdChrome: reports.hold.chrome.pass === true,
      svgChrome: reports.svg.chrome.pass === true,
      plainChrome: reports.plain.chrome.pass === true,
      overChrome: reports.over.chrome.pass === true,
      padChrome: reports.pad.chrome.pass === true,
      sentenceChrome: reports.sentence.chrome.pass === true,
      bareChrome: reports.bare.chrome.pass === true,
      payChrome: reports.pay.chrome.pass === true,
      passWallet: wallet("pass") === "skipped-gate",
      cleanWallet: wallet("clean") === "skipped-no-affordance",
      passPrinciples: status.pass.topics["design-principles"]?.state === "pending",
      remaining: reports.pass.plan.coverage.remaining > 0,
      fixWallet: wallet("fix") === "applied",
      markerGone: !/data-wl-strip(?![\w-])/.test(fixed),
      passKept: /\bdata-wallet\b/.test(fixed) && />\s*Pass\s*</.test(fixed),
      importKept: fs.readFileSync(path.join(fixDir, "Pass.swift"), "utf8").includes("import PassKit"),
      holdUnchanged: held === origHold,
      holdWallet: wallet("hold") === "pending",
      holdPhrase: /embedding text in the strip image/.test(held),
      svgUnchanged: svg === origSvg,
      svgWallet: wallet("svg") === "pending",
      svgTextKept: /<text>20% OFF<\/text>/.test(svg),
      plainWallet: wallet("plain") === "already-compliant",
      overWallet: wallet("over") === "already-compliant",
      padWallet: wallet("pad") === "already-compliant",
      sentenceWallet: wallet("sentence") === "skipped-no-affordance",
      bareWallet: wallet("bare") === "skipped-gate",
      bareMarkerRemains: /data-wl-strip(?![\w-])/.test(bared),
      payWallet: wallet("pay") === "skipped-gate",
      payMarkerRemains: /data-wl-strip(?![\w-])/.test(paid),
      noKit: !/SF Pro|-apple-system|shadcn/i.test(hostText),
    };
    ok = Object.values(checks).every(Boolean);
    detail = {
      passWallet: wallet("pass"),
      cleanWallet: wallet("clean"),
      fixWallet: wallet("fix"),
      holdWallet: wallet("hold"),
      svgWallet: wallet("svg"),
      plainWallet: wallet("plain"),
      overWallet: wallet("over"),
      padWallet: wallet("pad"),
...

{
  ...
  let detail = {};
  const passDir = fs.mkdtempSync(path.join(os.tmpdir(), "hig-apply-acs-pass-"));
  const cleanDir = fs.mkdtempSync(path.join(os.tmpdir(), "hig-apply-acs-clean-"));
  const fixDir = fs.mkdtempSync(path.join(os.tmpdir(), "hig-apply-acs-fix-"));
  const holdDir = fs.mkdtempSync(path.join(os.tmpdir(), "hig-apply-acs-hold-"));
  const waitDir = fs.mkdtempSync(path.join(os.tmpdir(), "hig-apply-acs-wait-"));
  const adsDir = fs.mkdtempSync(path.join(os.tmpdir(), "hig-apply-acs-ads-"));
  const sentenceDir = fs.mkdtempSync(path.join(os.tmpdir(), "hig-apply-acs-sentence-"));
  const bareDir = fs.mkdtempSync(path.join(os.tmpdir(), "hig-apply-acs-bare-"));
  const dirs = [passDir, cleanDir, fixDir, holdDir, waitDir, adsDir, sentenceDir, bareDir];
  const entitlement = `<?xml version="1.0" encoding="UTF-8"?>
<plist version="1.0"><dict>
  <key>com.apple.developer.associated-appclip-app-identifiers</key>
  <array><string>$(AppIdentifierPrefix)com.example.clip</string></array>
...
`;
  try {
    const src = path.join(pluginRoot, "eval", "fixtures", "chrome-pass");
    for (const dir of dirs) fs.cpSync(src, dir, { recursive: true });
    for (const dir of [cleanDir, fixDir, holdDir, waitDir, adsDir, sentenceDir]) {
      fs.writeFileSync(path.join(dir, "App.entitlements"), entitlement);
    }
    const codeOnly = `export function HostWidgets() {
  return (
...
`;
    ...
    const marked = `export function HostWidgets() {
  return (
    <div data-app-clip-code data-ac-splash>
      <button type="button">Code</button>
    </div>
  );
}
`;
  ...
  return (
    <div data-app-clip-code>
      <div className="splash">Please wait</div>
      <button type="button">Code</button>
    </div>
  );
}
`;
    fs.writeFileSync(path.join(holdDir, "HostWidgets.tsx"), origHold);
    const origWait = `export function HostWidgets() {
  return (
    <div data-app-clip-code>
      <p>Please wait</p>
      <button type="button">Code</button>
...
`;
    fs.writeFileSync(path.join(waitDir, "HostWidgets.tsx"), origWait);
    const origAds = `export function HostWidgets() {
  return (
    <div data-app-clip-code>
      <p>Don't display ads in your App Clip.</p>
...
`;
    fs.writeFileSync(path.join(adsDir, "HostWidgets.tsx"), origAds);
    const origSentence = `export function HostWidgets() {
  return <p>App Clips need to include all required assets, omit splash screens, and never make people wait on launch.</p>;
}
`;
    fs.writeFileSync(path.join(sentenceDir, "HostWidgets.tsx"), origSentence);
    const names = ["pass", "clean", "fix", "hold", "wait", "ads", "sentence", "bare"];
    const dirBy = {
      pass: passDir,
      clean: cleanDir,
      fix: fixDir,
    ...
    const checks = {
      requiredIds: loadSurfaces(skillRoot).requiredIds.length === 12,
      heuristic: (catalog.byId["app-clips"]?.dontHeuristicIds || []).includes("ac-splash"),
      fetchHeuristic: (catalog.byId["app-clips"]?.dontHeuristicIds || []).includes("ac-fetch"),
      passChrome: names.every((name) => reports[name].chrome.pass === true),
      passClip: clip("pass") === "skipped-gate",
      cleanClip: clip("clean") === "already-compliant",
      passPrinciples: status.pass.topics["design-principles"]?.state === "pending",
      remaining: reports.pass.plan.coverage.remaining > 0,
      fixClip: clip("fix") === "applied",
      markerGone: !/data-ac-splash(?![\w-])/.test(fixed),
      codeKept: /\bdata-app-clip-code\b/.test(fixed) && />\s*Code\s*</.test(fixed),
      holdUnchanged: held === origHold,
      holdClip: clip("hold") === "pending",
      splashKept: /className="splash"/.test(held),
      waitClip: clip("wait") === "already-compliant",
      adsUnchanged: ads === origAds,
      adsClip: clip("ads") === "already-compliant",
      sentenceClip: clip("sentence") === "skipped-no-affordance",
      bareClip: clip("bare") === "skipped-gate",
      bareMarkerRemains: /data-ac-splash(?![\w-])/.test(bared),
      noKit: !/SF Pro|-apple-system|shadcn/i.test(hostText),
    };
    ok = Object.values(checks).every(Boolean);
    detail = {
  ...
  results.push({ case: "catalog-apply-app-clip-splash-donts", ok, ...detail });
}

{
  let ok = false;
  let detail = {};
  const passDir = fs.mkdtempSync(path.join(os.tmpdir(), "hig-apply-pep-pass-"));
  const stableDir = fs.mkdtempSync(path.join(os.tmpdir(), "hig-apply-pep-stable-"));
  const holdDir = fs.mkdtempSync(path.join(os.tmpdir(), "hig-apply-pep-hold-"));
  const fixDir = fs.mkdtempSync(path.join(os.tmpdir(), "hig-apply-pep-fix-"));
  const sentenceDir = fs.mkdtempSync(path.join(os.tmpdir(), "hig-apply-pep-sentence-"));
  const bareDir = fs.mkdtempSync(path.join(os.tmpdir(), "hig-apply-pep-bare-"));
  const phoneDir = fs.mkdtempSync(path.join(os.tmpdir(), "hig-apply-pep-phone-"));
  const dirs = [passDir, stableDir, holdDir, fixDir, sentenceDir, bareDir, phoneDir];
  try {
    const src = path.join(pluginRoot, "eval", "fixtures", "chrome-pass");
    for (const dir of dirs) fs.cpSync(src, dir, { recursive: true });
    const writeIpad = (dir) => {
    ...
    for (const dir of [stableDir, holdDir, fixDir, sentenceDir]) writeIpad(dir);
    fs.writeFileSync(
      path.join(phoneDir, "DESIGN.md"),
      "platform_primary: phone\nregister: product\nThis product is an iPhone app.\n",
    );
    ...
    const stable = `export function HostWidgets() {
  return (
    <div data-pencil onPencilHover={() => setPreview(true)}>
...
`;
    ...
    const origHold = `export function HostWidgets() {
  return (
    <div data-pencil onPencilHover={(distance) => preview({ scale: distance })}>
      <div className="preview" />
      <button type="button">Ink</button>
    </div>
  );
...
`;
    ...
    const marked = `export function HostWidgets() {
  return (
    <div data-pencil data-pe-preview>
      <div className="preview" />
      <button type="button">Ink</button>
    </div>
  );
...
`;
    ...
    fs.writeFileSync(path.join(phoneDir, "HostWidgets.tsx"), marked);
    const origSentence = `export function HostWidgets() {
  return <p>Avoid continuously modifying the preview as people move Apple Pencil closer or farther from the screen.</p>;
}
`;
    fs.writeFileSync(path.join(sentenceDir, "HostWidgets.tsx"), origSentence);
    const names = ["pass", "stable", "hold", "fix", "sentence", "bare", "phone"];
    const dirBy = {
      pass: passDir,
      stable: stableDir,
      hold: holdDir,
    ...
    const checks = {
      requiredIds: loadSurfaces(skillRoot).requiredIds.length === 12,
      heuristic: (catalog.byId["apple-pencil-and-scribble"]?.dontHeuristicIds || []).includes("pe-preview"),
      squeeze: (catalog.byId["apple-pencil-and-scribble"]?.dontHeuristicIds || []).includes("pe-squeeze"),
      passChrome: names.every((name) => reports[name].chrome.pass === true),
      passPencil: pencil("pass") === "skipped-gate",
      passPrinciples: status.pass.topics["design-principles"]?.state === "pending",
      remaining: reports.pass.plan.coverage.remaining > 0,
      ...
      holdUnchanged: held === origHold,
      holdPencil: pencil("hold") === "pending",
      previewKept: /className="preview"/.test(held) && /scale: distance/.test(held),
      fixPencil: pencil("fix") === "applied",
      markerGone: !/data-pe-preview(?![\w-])/.test(fixed),
      inkKept: /\bdata-pencil\b/.test(fixed) && />\s*Ink\s*</.test(fixed) && /className="preview"/.test(fixed),
      sentencePencil: pencil("sentence") === "skipped-no-affordance",
      barePencil: pencil("bare") === "skipped-gate",
      bareMarkerRemains: /data-pe-preview(?![\w-])/.test(bared),
      phonePencil: pencil("phone") === "skipped-gate",
      phoneMarkerRemains: /data-pe-preview(?![\w-])/.test(phoned),
      noCanvasInvented: !/PKCanvasView|PKToolPicker|UIScribbleInteraction/.test(fixed + held),
      noKit: !/SF Pro|-apple-system|shadcn/i.test(hostText),
    };
    ok = Object.values(checks).every(Boolean);
...

{
  let ok = false;
  let detail = {};
  const passDir = fs.mkdtempSync(path.join(os.tmpdir(), "hig-apply-acg-pass-"));
  const cleanDir = fs.mkdtempSync(path.join(os.tmpdir(), "hig-apply-acg-clean-"));
  const spacedDir = fs.mkdtempSync(path.join(os.tmpdir(), "hig-apply-acg-spaced-"));
  const holdDir = fs.mkdtempSync(path.join(os.tmpdir(), "hig-apply-acg-hold-"));
  const graphicDir = fs.mkdtempSync(path.join(os.tmpdir(), "hig-apply-acg-graphic-"));
  const copyDir = fs.mkdtempSync(path.join(os.tmpdir(), "hig-apply-acg-copy-"));
  const fixDir = fs.mkdtempSync(path.join(os.tmpdir(), "hig-apply-acg-fix-"));
  const adsDir = fs.mkdtempSync(path.join(os.tmpdir(), "hig-apply-acg-ads-"));
  const sentenceDir = fs.mkdtempSync(path.join(os.tmpdir(), "hig-apply-acg-sentence-"));
  const bareDir = fs.mkdtempSync(path.join(os.tmpdir(), "hig-apply-acg-bare-"));
  const dirs = [passDir, cleanDir, spacedDir, holdDir, graphicDir, copyDir, fixDir, adsDir, sentenceDir, bareDir];
  const entitlement = `<?xml version="1.0" encoding="UTF-8"?>
<plist version="1.0"><dict>
  <key>com.apple.developer.associated-appclip-app-identifiers</key>
  <array><string>$(AppIdentifierPrefix)com.example.clip</string></array>
...
`;
  try {
    const src = path.join(pluginRoot, "eval", "fixtures", "chrome-pass");
    for (const dir of dirs) fs.cpSync(src, dir, { recursive: true });
    for (const dir of [cleanDir, spacedDir, holdDir, graphicDir, copyDir, fixDir, adsDir, sentenceDir]) {
      fs.writeFileSync(path.join(dir, "App.entitlements"), entitlement);
    }
    const codeOnly = `export function HostWidgets() {
  return (
...
`;
    fs.writeFileSync(path.join(cleanDir, "HostWidgets.tsx"), codeOnly);
    const spaced = `export function HostWidgets() {
  return (
    <div>
      <div data-app-clip-code style={{ margin: 16 }}>
        <button type="button">Code</button>
      </div>
      <div data-app-clip-code style={{ margin: 16 }}>
        <button type="button">Code</button>
...
`;
    fs.writeFileSync(path.join(spacedDir, "HostWidgets.tsx"), spaced);
    const origHold = `export function HostWidgets() {
  return (
    <div>
      <div data-app-clip-code>
...
`;
    fs.writeFileSync(path.join(holdDir, "HostWidgets.tsx"), origHold);
    const origGraphic = `export function HostWidgets() {
  return (
    <div>
      <div data-app-clip-code style={{ margin: 0 }}>
        <button type="button">Code</button>
      </div>
      <div className="graphic">Mark</div>
    </div>
  );
}
`;
    fs.writeFileSync(path.join(graphicDir, "HostWidgets.tsx"), origGraphic);
    const origCopy = `export function HostWidgets() {
  return (
    <div data-app-clip-code>
      <p>Provide enough space between an App Clip Code and adjacent App Clip Codes.</p>
      <button type="button">Code</button>
    </div>
  );
}
`;
    ...
    const marked = `export function HostWidgets() {
  return (
    <div data-app-clip-code data-ac-gap>
      <button type="button">Code</button>
    </div>
  );
}
...
`;
    fs.writeFileSync(path.join(adsDir, "HostWidgets.tsx"), origAds);
    const origSentence = `export function HostWidgets() {
  return <p>Provide enough space between an App Clip Code and adjacent App Clip Codes.</p>;
}
`;
    fs.writeFileSync(path.join(sentenceDir, "HostWidgets.tsx"), origSentence);
    const names = ["pass", "clean", "spaced", "hold", "graphic", "copy", "fix", "ads", "sentence", "bare"];
    const dirBy = {
      pass: passDir,
      clean: cleanDir,
      spaced: spacedDir,
    ...
    const fixed = fs.readFileSync(path.join(fixDir, "HostWidgets.tsx"), "utf8");
    const held = fs.readFileSync(path.join(holdDir, "HostWidgets.tsx"), "utf8");
    const graphic = fs.readFileSync(path.join(graphicDir, "HostWidgets.tsx"), "utf8");
    const copied = fs.readFileSync(path.join(copyDir, "HostWidgets.tsx"), "utf8");
    const ads = fs.readFileSync(path.join(adsDir, "HostWidgets.tsx"), "utf8");
    const bared = fs.readFileSync(path.join(bareDir, "HostWidgets.tsx"), "utf8");
    const hostText = dirs.flatMap((dir) => walkSource(dir)).map((f) => f.text).join("\n");
    ...
    const checks = {
      requiredIds: loadSurfaces(skillRoot).requiredIds.length === 12,
      heuristic: (catalog.byId["app-clips"]?.dontHeuristicIds || []).includes("ac-gap"),
      small: (catalog.byId["app-clips"]?.dontHeuristicIds || []).includes("ac-small"),
      passChrome: names.every((name) => reports[name].chrome.pass === true),
      passClip: clip("pass") === "skipped-gate",
      passPrinciples: status.pass.topics["design-principles"]?.state === "pending",
      remaining: reports.pass.plan.coverage.remaining > 0,
      cleanClip: clip("clean") === "already-compliant",
      spacedClip: clip("spaced") === "already-compliant",
      holdUnchanged: held === origHold,
      holdClip: clip("hold") === "pending",
      codesKept: (held.match(/\bdata-app-clip-code\b/g) || []).length === 2,
      graphicUnchanged: graphic === origGraphic,
      graphicClip: clip("graphic") === "pending",
      graphicKept: /className="graphic"/.test(graphic) && /margin: 0/.test(graphic),
      copyUnchanged: copied === origCopy,
      copyClip: clip("copy") === "pending",
      fixClip: clip("fix") === "applied",
      markerGone: !/data-ac-gap(?![\w-])/.test(fixed),
      codeKept: /\bdata-app-clip-code\b/.test(fixed) && />\s*Code\s*</.test(fixed),
      adsUnchanged: ads === origAds,
      adsClip: clip("ads") === "already-compliant",
      sentenceClip: clip("sentence") === "skipped-no-affordance",
      bareClip: clip("bare") === "skipped-gate",
      bareMarkerRemains: /data-ac-gap(?![\w-])/.test(bared),
      noKit: !/SF Pro|-apple-system|shadcn/i.test(hostText),
    };
    ok = Object.values(checks).every(Boolean);
    detail = {
      passClip: clip("pass"),
      cleanClip: clip("clean"),
      spacedClip: clip("spaced"),
      holdClip: clip("hold"),
      graphicClip: clip("graphic"),
      copyClip: clip("copy"),
      fixClip: clip("fix"),
      adsClip: clip("ads"),
      sentenceClip: clip("sentence"),
  ...
  results.push({ case: "catalog-apply-app-clip-gap-donts", ok, ...detail });
}
