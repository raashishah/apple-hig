...
After apply, `/hig` runs `node skills/hig/scripts/check-chrome.mjs` on the host. P0 hits block PASS. Dual-stack P0s `chrome.view-mode.icons` and `chrome.bars.system-materials` are applied by [`skills/hig/scripts/apply-chrome.mjs`](skills/hig/scripts/apply-chrome.mjs) (no kit, no font rewrite). Other recipes: [`skills/hig/knowledge/chrome/recipes.md`](skills/hig/knowledge/chrome/recipes.md). Host fonts stay. Live Apple article inventory: [`skills/hig/knowledge/catalog.yaml`](skills/hig/knowledge/catalog.yaml) (not the round-1 apply list). After chrome is clean, `scripts/plan-catalog.mjs` continues applicable packed topics until remaining is 0. Pattern surfaces skip when the host has no matching widget. Catalog apply rules are derived at load from chrome grammar and pack Do/Don't, not title stubs. The same chrome FAILs are proven on a web host and a Swift host; the rule text does not name those stacks.
