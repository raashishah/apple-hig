...
3. Load relevant pattern packs (Chrome gates = IDs only)
...
6. Match chrome `failWhen` lines. Emit `structure:<rule.id>` for each hit
...
10. Verdict **FAIL** if checker `pass` is false, even when screenshots are missing.
