...
- **Chrome grammar** (`knowledge/chrome/grammar.yaml` + `scripts/check-chrome.mjs`): cite `structure:<rule.id>` when `failWhen` matches or the scanner reports it. Missing screenshots do not imply PASS if the scanner FAILs.
